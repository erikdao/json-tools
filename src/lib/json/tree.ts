...
export function buildTree(src: string): { ok: true; tree: TreeNode } | { ok: false; error: JsonError } {
  try { return { ok: true, tree: build(parseRaw(src), null, '') }; }
  catch (e) { return { ok: false, error: normalizeJsonError(src, e as Error) }; }
}
